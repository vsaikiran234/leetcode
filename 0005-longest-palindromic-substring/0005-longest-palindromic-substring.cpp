class Solution { ///class Solution {
 ...
 public: //public:
  string longestPalindrome(string s) {   // string LongestPalindrome(string s )
    ...
    pair<int, int> indices{0, 0}; // pair<int, int> indices[0, 0]

    for (int i = 0; i < s.length(); ++i) { // for ( int i = 0 ;; i < s.legth(); ++i)
      const auto [l1, r1] = extend(s, i, i); // const auto [l1, r1] = extend(s, i, i);
