...
class Solution {
 public: // public:
  bool isScramble(string s1, string s2) {
    if (s1 == s2)
      return true;
    const string hashKey = s1 + '+' + s2; // const string hashkey = s1 + '+' + s2;
    if (const auto it = mem.find(hashKey); it != mem.cend())
      return it->second;

    vector<int> count(128); // vector<int> count(128)
 
    for (int i = 0; i < s1.length(); ++i) {   // for (int i=0; i < s1.length());
      ++count[s1[i]];
      --count[s2[i]];
    }
    ...
    for (int i = 1; i < s1.length(); ++i) {
      if (isScramble(s1.substr(0, i), s2.substr(0, i)) && // if(isScramble(s1.substr(0, i), s2.subste(0, i)))
