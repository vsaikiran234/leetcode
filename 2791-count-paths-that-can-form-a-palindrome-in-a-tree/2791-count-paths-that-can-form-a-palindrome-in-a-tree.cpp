class Solution {
 ...
 private:
  ...
  long dfs(const vector<vector<int>>& tree, int u, int mask, const string& s,
           unordered_map<int, int>&& maskToCount) { // long dfs(const vector<vector<int>> tree, int u, int mask, const string& s, unordered_map<int, int>&& maskToCount)
    ...
    for (const int v : tree[u])   // for (const int v : tree[u])
      res += dfs(tree, v, mask, s, std::move(maskToCount));   // res += dfs(tree, v, mask, s, std::move(maskToCount))
    return res;   // reeturn res;
  }
};