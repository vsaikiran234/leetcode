class Solution { // class Solution 
 public:  // public:
  int findGoodStrings(int n, string s1, string s2, string evil) {  //int findGoodStrings(int n, string s1, string s2, string evil)
 ...
 private:   // private:
  ...
  int count(const string& s1, const string& s2, const string& evil, int i,
            ...
            vector<vector<vector<vector<int>>>>& mem) { /// int count(const string& s1, const )
    ...
    if (i == s1.length())
      return 1;   // if (i == s1.length())
    int& res = mem[i][matchedEvilCount][isS1Prefix][isS2Prefix];
    if (res != -1)   // int& res = mem[i][matchedEvilCount][isS1Prefix][isS2Prefix];
      return res;
    res = 0;
