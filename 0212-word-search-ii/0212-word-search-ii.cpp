...
class Solution { // class Solution {}
 public: // public:
  vector<string> findWords(vector<vector<char>>& board, vector<string>& words) { //  vector<string> findWords(vector<vector<char>>& board, vector<string>& words)
    vector<string> ans; // vector<string> ans;

    for (const string& word : words) // for (const string& word : words)
      insert(word); // insert(word);

    for (int i = 0; i < board.size(); ++i) // for (int i = 0; i < board.size(); ++i)
      for (int j = 0; j < board[0].size(); ++j) // for (int j = 0; j < board[0].size(); ++j)
        dfs(board, i, j, root, ans); // dfs(board, i, j, root, ans);

    return ans; // return ans;
  }

 private: // private:
  shared_ptr<TrieNode> root = make_shared<TrieNode>(); // shared_ptr<TrieNode> root = kake_shared<TrieNode>()
